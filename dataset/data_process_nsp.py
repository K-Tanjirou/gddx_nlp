...
if __name__ == '__main__':
    ...
    train_chosen_positive = data.loc[train_chosen_index, ['prompt', 'answer1', 'answer2', 'answer1_target', 'tfidf']]
    train_chosen_negative = data.loc[train_chosen_index, ['prompt', 'answer2', 'answer1', 'answer2_target', 'tfidf']]

    train_reject_positive = data.loc[train_reject_index, ['prompt', 'answer2', 'answer1', 'answer2_target', 'tfidf']]
    train_reject_negative = data.loc[train_reject_index, ['prompt', 'answer1', 'answer2', 'answer1_target', 'tfidf']]

    test_chosen_positive = data.loc[test_chosen_index, ['prompt', 'answer1', 'answer2', 'answer1_target', 'tfidf']]
    test_chosen_negative = data.loc[test_chosen_index, ['prompt', 'answer2', 'answer1', 'answer2_target', 'tfidf']]

    test_reject_positive = data.loc[test_reject_index, ['prompt', 'answer2', 'answer1', 'answer2_target', 'tfidf']]
    test_reject_negative = data.loc[test_reject_index, ['prompt', 'answer1', 'answer2', 'answer1_target', 'tfidf']]

    # train_data = pd.concat([train_data_chosen, train_data_reject])
    # test_data = pd.concat([test_data_chosen, test_data_reject])

    train_data = pd.DataFrame(np.concatenate([
        train_chosen_positive.values,
        train_chosen_negative.values,
        train_reject_positive.values,
        train_reject_negative.values,
    ], axis=0), columns=['prompt', 'answer1', 'answer2', 'target', 'tfidf'])

    test_data = pd.DataFrame(np.concatenate([
        test_chosen_positive.values,
        test_chosen_negative.values,
        test_reject_positive.values,
        test_reject_negative.values,
    ], axis=0), columns=['prompt', 'answer1', 'answer2', 'target', 'tfidf'])

    print(len(train_data))
    print(len(test_data))

    train_data.to_csv('/root/competition/dataset/nsp/train_data.csv', sep='\t')
    test_data.to_csv('/root/competition/dataset/nsp/test_data.csv', sep='\t')
